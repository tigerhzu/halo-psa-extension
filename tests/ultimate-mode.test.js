...
assert.deepEqual(
  ...
  ['Op Team A', 'Op Team B', 'Op Team C', 'Timesheets']
);
assert.deepEqual(Array.from(cfg.SIDEBAR.TEAM_ITEMS), ['Op Team A', 'Op Team B', 'Op Team C']);
...
assert.ok(sharedIndex !== -1 && sharedIndex < indexIndex && indexIndex < entryIndex, '極致模式 manifest 載入順序錯誤');
...
assert.match(read('src/ultimate-mode/team-shortcuts.js'), /chrome\.runtime\.getURL\(assetPath\)/, '四個按鈕必須使用 Extension 內建圖片 logo');
...
assert.match(read('src/ultimate-mode/ultimate.css'), /data-hpx-ultimate-mode="on"[^}]*#halo-tree/s, 'Sidebar 必須有第一幀 CSS guard');
